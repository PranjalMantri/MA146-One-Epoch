        ...
        if id in vh_down:
            if cy2 < (cy + offset) and cy2 > (cy - offset):
                ...
                if counter.count(id) == 0:
                    ...
                    car_speeds_down[id] = int(a_speed_kh)
                    cv2.circle(frame, (cx, cy), 4, (0, 0, 255), -1)
                    cv2.putText(frame, str(id), (x3, y3), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 1)
                    cv2.putText(frame, str(int(a_speed_kh)) + 'Km/h', (x4, y4), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (0, 255, 255), 2)

        if id in car_speeds_down:
            cv2.putText(frame, str(car_speeds_down[id]) + 'Km/h', (x4, y4), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                        (0, 255, 255), 2)
        ...
        if id in vh_up:
            if cy1 < (cy + offset) and cy1 > (cy - offset):
                ...
                if counter1.count(id) == 0:
                    ...
                    car_speeds_up[id] = int(a_speed_kh1)
                    cv2.circle(frame, (cx, cy), 4, (0, 0, 255), -1)
                    cv2.putText(frame, str(id), (x3, y3), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 1)
                    cv2.putText(frame, str(int(a_speed_kh1)) + 'Km/h', (x4, y4), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                (0, 255, 255), 2)

        if id in car_speeds_up:
            cv2.putText(frame, str(car_speeds_up[id]) + 'Km/h', (x4, y4), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 255),
                        2)
